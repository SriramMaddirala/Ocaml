open Lwt
(* Functionality I added is Private message between Users; to trigger it simply start message with /p then name of receiver then message : i.e. : /p Alice hi
Doesn't require any modules apart from the ones already in use by chatServer *)
...
Lwt.return ()
(*Parse through line and return a tuple of person receiving message and the message*)
let parse_input_message strwant = 
let trimString = String.trim strwant 
in let index = String.index trimString ' '
in let sendto = Str.string_before trimString index in let msg = Str.string_after trimString (index+1) in
(sendto,msg)
(* send message to a given receiver from a sender*)
let sendmessage sender sendto message = Lwt_list.iter_p (fun l ->(match l with
| (a,b) -> if(a = sendto) then Lwt_io.fprintl b ((("Private Message from  " ^ (sender)) ^ " : ") ^ message)  else Lwt.return ())) !sessions
...
| "/p" -> let strwant = (Str.string_after l 3) in let (sendto, message) = parse_input_message strwant in sendmessage !nr sendto message >>= fun () -> Lwt.return ()
| _ -> broadcast !nr l
